...
const myMaterials = computed(() =>
  catalog.products.filter((product) =>
    ...
    (product.accessType === 'free' || catalog.purchasedIds.includes(product.id))),
)
...
const syncPaidExamAccess = async () => {
  ...
  accessResponses.forEach((result, index) => {
    if (result.status === 'fulfilled' && result.value.data.has_access) {
      catalog.buyProduct(paidExams[index].id)
    }
  })
}
...
const startExam = async (examId: string) => {
  ...
  try {
    ...
    if (exam.accessType === 'paid' && !catalog.purchasedIds.includes(exam.id)) {
...
const viewMaterial = async (materialId: string) => {
  try {
    await catalog.fetchMaterialDetails(materialId)
  ...
  <div class="space-y-6">
    ...
    <section v-if="viewMode === 'dashboard'" class="grid gap-6">
      ...
      <div v-else class="grid gap-6 lg:grid-cols-[1.3fr_0.7fr]">
            ...
            <article v-for="exam in filteredExams" :key="exam.id" class="rounded-2xl border border-slate-100 p-5">
              <div class="flex flex-wrap items-center justify-between gap-3">
                ...
                <div class="text-right">
                  ...
                  <p v-if="exam.accessType === 'paid'" class="text-[11px] text-slate-400">{{ catalog.purchasedIds.includes(exam.id) ? 'Unlocked' : 'Locked' }}</p>
                </div>
              </div>
              <p class="mt-3 text-sm text-slate-600">{{ exam.description }}</p>
              <div class="mt-4 flex flex-wrap gap-3">
                <button
                  v-if="exam.accessType === 'free' || catalog.purchasedIds.includes(exam.id)"
        ...
        <aside class="space-y-6">
          ...
          <section v-if="myMaterials.length" class="rounded-[2rem] border border-slate-100 bg-white p-8 shadow-sm">
            <h3 class="text-lg font-black text-slate-900">My Materials</h3>
            <div class="mt-5 space-y-3">
              <button v-for="material in myMaterials" :key="material.id" class="flex w-full items-center justify-between rounded-2xl bg-slate-50 px-4 py-3 text-left" @click="void viewMaterial(material.id)">
                <span class="text-sm font-bold text-slate-900">{{ material.title }}</span>
                <span class="text-xs font-semibold text-slate-500">{{ material.category }}</span>
              </button>
            </div>
          </section>
        </aside>
    ...
    <section v-else-if="viewMode === 'viewer' && selectedMaterial" class="rounded-[2rem] border border-slate-100 bg-white p-8 shadow-sm">
      <div class="flex items-center justify-between gap-4">
        ...
        <button class="rounded-full border border-slate-200 px-5 py-2 text-sm font-semibold text-slate-700" @click="viewMode = 'dashboard'">Close Viewer</button>
      </div>
      <div class="mt-8 rounded-[2rem] bg-slate-900 px-6 py-10 text-center text-white">
        <p class="text-sm font-semibold uppercase tracking-[0.18em] text-slate-300">Protected material</p>
        <p class="mt-4 text-lg font-bold">{{ selectedMaterial.downloadUrl }}</p>
      </div>
    </section>

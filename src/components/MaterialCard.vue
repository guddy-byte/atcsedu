...
const openPurchasedAccess = () => {
  if (props.product.type === 'material') {
    void router.push({
      path: '/exam-training',
      query: {
        view: 'viewer',
        material: props.product.id,
      },
    })
    return
  }

  void router.push('/exam-training')
}
</script>
    ...
    <div class="grid gap-2.5 rounded-[1rem] bg-rose-50/70 p-3.5">
      ...
      <div class="flex items-center justify-between gap-3 border-t border-rose-100 pt-2.5">
        ...
        <button
          ...
          :disabled="isPaying"
          @click="emit('pay', { productId: product.id, productType: product.type })"
        >
